...
class Store(Protocol):
    ...
    def release_lease(self, job_id: str, owner: str, now: datetime) -> None:
        """Hands a job back after a graceful stop: clears the lease, makes the
        job claimable now and refunds the attempt its claim added (floored at
        0). A no-op unless `owner` still holds the lease."""
        ...
